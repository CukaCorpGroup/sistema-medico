    ...
    <div class="gloves-page">
      <div class="form-container">
        ...
        <div class="form-actions">
          <button type="button" class="btn btn-secondary" (click)="loadLastData()" [disabled]="!patientData.id || loadingLastData">
            <i class="fas fa-spinner fa-spin" *ngIf="loadingLastData"></i>
            <i class="fas fa-history" *ngIf="!loadingLastData"></i>
            {{ loadingLastData ? 'CARGANDO...' : 'Cargar ultimos datos' }}
          </button>
          ...
          <button type="button" class="btn btn-success" (click)="saveGlovesRecord()" [disabled]="saving">
            <i class="fas fa-spinner fa-spin" *ngIf="saving"></i>
            {{ saving ? 'GUARDANDO...' : 'Guardar uso de guantes' }}
          </button>
        </div>
...
export class GlovesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sharedPatientService.patientData$.subscribe(sharedPatient => {
      if (sharedPatient) {
        console.log('GlovesComponent: Recibió actualización del servicio compartido', sharedPatient);
    ...
    setTimeout(() => {
      ...
      console.log('GlovesComponent: Datos compartidos al iniciar', sharedPatient);
      if (sharedPatient) {
        this.loadSharedPatientData(sharedPatient);
      }
    }, 200);
  ...
  loadSharedPatientData(sharedPatient: any) {
    if (!sharedPatient) {
      console.log('GlovesComponent: No hay datos del paciente compartido');
      return;
    }
    
    console.log('GlovesComponent: Cargando datos del paciente compartido', sharedPatient);
    ...
    console.log('GlovesComponent: Datos del paciente y consulta cargados correctamente');
  }
  ...
  selectCIE10(code: CIE10) {
    ...
    if (!this.formData.secondaryDescription) {
      this.formData.secondaryDescription = `Uso de guantes relacionado a ${code.description}`;
    }
  ...
  generateSecondaryCode(cie10Code: string): string {
    ...
    // Generar código secundario con formato: GLO-[CIE10]-[Año][Mes]
    const now = new Date();
    ...
    return `GLO-${cie10Prefix}-${year}${month}`;
  }
  ...
  generatePDF() {
    ...
    doc.text('REGISTRO DE USO DE GUANTES', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 10;
    ...
    // Fechas
    doc.setFont('helvetica', 'bold');
    doc.text('PERÍODO DE USO', margin, yPosition);
    yPosition += 6;
    ...
    // Guardar PDF
    const filename = `GUANTES_${this.patientData.identification}_${this.formData.date.replace(/-/g, '')}.pdf`;
    doc.save(filename);
  ...
  saveGlovesRecord() {
    ...
    this.http.post<any>(`${environment.apiUrl}/gloves`, glovesData)
      .subscribe({
        next: (response) => {
          this.saving = false;
          if (response.success) {
            this.successMessage = response.message || 'Registro de uso de guantes guardado exitosamente';
        ...
        error: (error) => {
          console.error('Error guardando registro de guantes:', error);
          this.saving = false;
          
          // Manejar diferentes tipos de errores
          if (error.status === 404) {
            this.errorMessage = error.error?.message || 'Empleado no encontrado en el sistema';
          } else if (error.status === 401) {
            this.errorMessage = 'No autorizado. Por favor inicie sesión nuevamente';
          } else if (error.status === 400) {
            this.errorMessage = error.error?.message || 'Datos inválidos. Por favor verifique la información';
          } else if (error.status === 500) {
            this.errorMessage = error.error?.message || 'Error del servidor. Por favor intente más tarde';
          } else {
            this.errorMessage = error.error?.message || error.message || 'Error al guardar el registro. Por favor intente nuevamente';
          }
        }
      });
  }
  ...
  loadLastData() {
    ...
    // Obtener la última atención médica del paciente
    this.http.get<any>(`${environment.apiUrl}/medical-records`, {
      ...
      next: (response) => {
        ...
        if (response.success && response.data && response.data.length > 0) {
          const lastRecord = response.data[0]; // El más reciente
          
          // Cargar datos del último registro médico
          if (lastRecord.cie10Code) {
            this.formData.cie10Code = lastRecord.cie10Code;
            this.formData.cie10Description = lastRecord.cie10Description || '';
          }
          
          if (lastRecord.causes) {
            this.formData.causes = lastRecord.causes;
          }
          
          if (lastRecord.diagnosis) {
            this.formData.evolution = lastRecord.diagnosis;
          }
          
          // Generar código secundario si no existe
          if (lastRecord.cie10Code && !this.formData.secondaryCode) {
            this.formData.secondaryCode = this.generateSecondaryCode(lastRecord.cie10Code);
          }
          
          if (lastRecord.cie10Description && !this.formData.secondaryDescription) {
            this.formData.secondaryDescription = `Uso de guantes relacionado a ${lastRecord.cie10Description}`;
          }
          ...
          if (currentShared) {
            const updatedShared = {
              ...currentShared,
              cie10Code: this.formData.cie10Code,
              cie10Description: this.formData.cie10Description,
              causes: this.formData.causes,
              diagnosis: this.formData.evolution,
              secondaryCode: this.formData.secondaryCode,
              secondaryDescription: this.formData.secondaryDescription
            };
            this.sharedPatientService.setPatientData(updatedShared);
          ...
          this.successMessage = 'Últimos datos de atención médica cargados exitosamente';
          setTimeout(() => {
            this.successMessage = '';
          }, 3000);
        } else {
      ...
      error: (error) => {
        console.error('Error cargando últimos datos:', error);
        this.loadingLastData = false;
        this.errorMessage = 'Error al cargar los últimos datos: ' + (error.error?.message || 'Error desconocido');
      }
    });
  }
  ...
  downloadHistory() {
    this.http.get(`${environment.apiUrl}/gloves/export/excel`, {
      responseType: 'blob'
    }).subscribe({
      next: (blob: Blob) => {
        ...
        link.href = url;
        link.download = `Historial_Uso_Guantes_${new Date().toISOString().split('T')[0]}.xlsx`;
