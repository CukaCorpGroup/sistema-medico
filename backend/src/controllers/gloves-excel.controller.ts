...
export const createGlovesRecord = async (req: Request, res: Response) => {
  try {
    ...
    const glovesRecord = await excelDB.createGloves({
    ...
    logger.info(`Registro de guantes creado en Excel: ${glovesRecord.id}`);
...
export const getGlovesRecords = async (req: Request, res: Response) => {
  try {
    ...
    let records = await excelDB.getGlovesRecords();
...
export const exportGlovesToExcel = async (req: Request, res: Response) => {
  try {
    ...
    const records = await excelDB.getGlovesRecords();
    const patients = await excelDB.getPatients();
    const users = await excelDB.getUsers();

    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Registros de Guantes');
